    ...
    <div className="bg-[#F8F8F8] min-h-screen p-8 font-sans">
      ...
      >
        {items.map((item) => (
          <div
            key={item.id}
            className="bg-white rounded-xl shadow hover:shadow-lg cursor-pointer border hover:border-[#C8102E] transition p-2"
            onClick={() => setSelected(item)}
          >
            <img
              src={item.image}
              alt={item.title}
              className="h-[200px] w-auto max-w-full mx-auto"
            />
          </div>
        ))}
      </div>
        ...
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50">
          <div className="bg-white rounded-lg p-6 w-[90%] max-w-lg shadow-lg relative">
            ...
            {selected.students.map((s, i) => (
              <div key={i} className="mb-3">
                ...
                {s.linkedin && (
                  <a
                    href={s.linkedin}
                    className="text-sm text-[#C8102E] block"
                    target="_blank"
                    rel="noreferrer"
                  >
                    LinkedIn
            ...
            <button
              ...
              className="mt-4 bg-[#C8102E] text-white px-4 py-2 rounded"
            >
              Close
            </button>
          </div>
